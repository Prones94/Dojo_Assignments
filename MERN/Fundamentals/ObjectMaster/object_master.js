...
const greaterThan39 = pokemon.filter(p => p.id > 39).map(p => p.name)
// console.log(greaterThan39);

const poison = pokemon.filter(p => p.types.includes("poison") && p.types.length === 1).map(p => p.name)
// console.log(poison);
